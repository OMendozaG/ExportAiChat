...
- Virtualized hydration now uses the same cycle across ChatGPT, Claude, Gemini, Grok, and DeepSeek: scroll to top, move down in overlapping viewport-sized immediate steps (0.5s between steps), wait 2s, move to top and wait 2s, force top every 1s until confirmed, then force bottom every 1s until confirmed
...
- TXT option to trim leading spaces/newlines on the first Human header (default on)
