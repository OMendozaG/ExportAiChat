...
- dedicated TXT export settings (separate from HTML/PDF/MHT appearance settings),
- multiline TXT continuation style,
- editable TXT message separator,
- editable TXT Human/AI per-message header templates with `<HumanName>`, `<AiName>`, and `<enter>` placeholders,
- dedicated HTML/PDF/MHT color settings for Human/AI role borders,
