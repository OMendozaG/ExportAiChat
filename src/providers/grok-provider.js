...
(() => {
  ...
  async function runWithHydratedMessageNodes(selector, options, worker) {
    ...
    try {
      ...
      if (!reachedTop) {
        throw new Error("HYDRATION_TOP_REACH_FAILED: Grok history did not reach top within timeout.");
      }
      ...
      while (!hydrationBudgetExceeded()) {
        ...
        if (!atTop) {
          ...
          if (!reachedTopAgain) {
            throw new Error("HYDRATION_TOP_REACH_FAILED: Grok history lost top position before completion.");
          }
          atTop = true;
        }
      ...
      if (hydrationBudgetExceeded()) {
        throw new Error("HYDRATION_TOP_REACH_FAILED: Grok history kept growing until hydration timeout.");
      }
